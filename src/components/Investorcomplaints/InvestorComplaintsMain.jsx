import React from "react";

import "./investorcomplaints.scss";

import "../CEBPLPolicies/CEBPL-Policies.scss";

function InvestorComplaintsMain() {
    return (
        <div>
            <section className="mainwrapquick">
                <div className="container">
                    <div className="row">
                        <div className="col-md-12">
                            <h2 className="title">Investor Complaints</h2>

                        </div>
                    </div>
                    <div className="row quicklinkswrap mb-5">
                        <div className="col-md-12">
                            <small>Trend of Monthly Disposal of Complaints</small>
                            <h4 className="text-center text-uppercase mt-5 mb-5"><strong>For the financial year 2020 - 2021</strong></h4>
                            <div className="table-responsive">
                            <table className="table table-striped" id="complaints">
                                <thead>
                                    <tr>
                                        <th className="text-uppercase ">sr. no</th>
                                        <th className="text-uppercase ">Month</th>
                                        <th className="text-uppercase text-center">Brought Forward<br/>
                                            ( previous month )</th>
                                        <th className="text-uppercase text-center">Received This<br/> month</th>
                                        <th className="text-uppercase text-center">resolved this<br/> month</th>
                                        <th className="text-uppercase text-center">pending</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
                                        <td >1</td>
                                        <td>April 2020</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">0</td>
                                    </tr>
                                    <tr>
                                        <td>2</td>
                                        <td>May 2020</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">0</td>
                                    </tr>
                                    <tr>
                                        <td>3</td>
                                        <td>June 2020</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">1</td>
                                        <td className="text-center">1</td>
                                        <td className="text-center">0</td>
                                    </tr>
                                    <tr>
                                        <td>4</td>
                                        <td>July 2020</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">3</td>
                                        <td className="text-center"> 1</td>
                                        <td className="text-center">2</td>
                                    </tr>
                                    <tr>
                                        <td>5</td>
                                        <td>August 2020</td>
                                        <td className="text-center">2</td>
                                        <td className="text-center">4</td>
                                        <td className="text-center">6</td>
                                        <td className="text-center">0</td>
                                    </tr>
                                    <tr>
                                        <td>6</td>
                                        <td>September 2020</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">21</td>
                                        <td className="text-center">11</td>
                                        <td className="text-center">10</td>
                                    </tr>
                                    <tr>
                                        <td>7</td>
                                        <td>October 2020</td>
                                        <td className="text-center">10</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">1</td>
                                        <td className="text-center">9</td>
                                    </tr>
                                    <tr>
                                        <td>8</td>
                                        <td>November 2020</td>
                                        <td className="text-center">9</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">9</td>
                                        <td className="text-center">0</td>
                                    </tr>
                                    <tr>
                                        <td>9</td>
                                        <td>December 2020</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">0</td>
                                    </tr>
                                    <tr>
                                        <td>10</td>
                                        <td>January 2021</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">0</td>
                                    </tr>
                                    <tr>
                                        <td>11</td>
                                        <td>February 2021</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">2</td>
                                        <td className="text-center">2</td>
                                        <td className="text-center" >0</td>
                                    </tr>
                                    <tr>
                                        <td>12</td>
                                        <td>March 2021</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">2</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">2</td>
                                    </tr>
                                </tbody>
                            </table>
                            </div>
                        </div>
                    </div>

                    <div className="row quicklinkswrap mb-5">
                        <div className="col-md-12">
                            <small>Trend of Monthly Disposal of Complaints</small>
                            <h4 className="text-center text-uppercase mt-5 mb-5"><strong>For the financial year 2021 - 2022</strong></h4>
                            <div className="table-responsive">
                            <table className="table table-striped" id="complaints">
                                <thead>
                                    <tr>
                                        <th className="text-uppercase ">sr. no</th>
                                        <th className="text-uppercase ">Month</th>
                                        <th className="text-uppercase text-center">Brought Forward<br/>
                                            ( previous month )</th>
                                        <th className="text-uppercase text-center">Received This<br/> month</th>
                                        <th className="text-uppercase text-center">resolved this<br/> month</th>
                                        <th className="text-uppercase text-center">pending</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
                                        <td >1</td>
                                        <td>April 2020</td>
                                        <td className="text-center">2</td>
                                        <td className="text-center">8</td>
                                        <td className="text-center">8</td>
                                        <td className="text-center">2</td>
                                    </tr>
                                    <tr>
                                        <td>2</td>
                                        <td>May 2020</td>
                                        <td className="text-center">2</td>
                                        <td className="text-center">4</td>
                                        <td className="text-center">4</td>
                                        <td className="text-center">2</td>
                                    </tr>
                                    <tr>
                                        <td>3</td>
                                        <td>June 2020</td>
                                        <td className="text-center">2</td>
                                        <td className="text-center">4</td>
                                        <td className="text-center">6</td>
                                        <td className="text-center">0</td>
                                    </tr>
                                    <tr>
                                        <td>4</td>
                                        <td>July 2020</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">0</td>
                                    </tr>
                                    <tr>
                                        <td>5</td>
                                        <td>August 2020</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">11</td>
                                        <td className="text-center">9</td>
                                        <td className="text-center">2</td>
                                    </tr>
                                    <tr>
                                        <td>6</td>
                                        <td>September 2020</td>
                                        <td className="text-center">2</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">2</td>
                                    </tr>
                                    <tr>
                                        <td>7</td>
                                        <td>October 2020</td>
                                        <td className="text-center">2</td>
                                        <td className="text-center">9</td>
                                        <td className="text-center">3</td>
                                        <td className="text-center">7</td>
                                    </tr>
                                    <tr>
                                        <td>8</td>
                                        <td>November 2020</td>
                                        <td className="text-center">6</td>
                                        <td className="text-center"></td>
                                        <td className="text-center"></td>
                                        <td className="text-center"></td>
                                    </tr>
                                    <tr>
                                        <td>9</td>
                                        <td>December 2020</td>
                                        <td className="text-center">7</td>
                                        <td className="text-center">14</td>
                                        <td className="text-center">11</td>
                                        <td className="text-center">10</td>
                                    </tr>
                                    <tr>
                                        <td>10</td>
                                        <td>January 2021</td>
                                        <td className="text-center">10</td>
                                        <td className="text-center">29</td>
                                        <td className="text-center">19</td>
                                        <td className="text-center">20</td>
                                    </tr>
                                    <tr>
                                        <td>11</td>
                                        <td>February 2021</td>
                                        <td className="text-center">20</td>
                                        <td className="text-center">15</td>
                                        <td className="text-center">34</td>
                                        <td className="text-center" >1</td>
                                    </tr>
                                    <tr>
                                        <td>12</td>
                                        <td>March 2021</td>
                                        <td className="text-center">1</td>
                                        <td className="text-center">5</td>
                                        <td className="text-center">5</td>
                                        <td className="text-center">1</td>
                                    </tr>
                                </tbody>
                            </table>
                            </div>
                        </div>
                    </div>

                    <div className="row quicklinkswrap">
                        <div className="col-md-12">
                            <small>annual Disposal of Complaints</small>
                            <h4 className="text-center text-uppercase mt-5 mb-5"><strong>For the financial year 2021 - 2022</strong></h4>
                            <div className="table-responsive">
                            <table className="table table-striped" id="complaints">
                                <thead>
                                    <tr>
                                        <th className="text-uppercase ">sr. no</th>
                                        <th className="text-uppercase ">YEAR</th>
                                        <th className="text-uppercase text-center">Brought Forward<br/>
                                            ( previous month )</th>
                                        <th className="text-uppercase text-center">Received This<br/> month</th>
                                        <th className="text-uppercase text-center">resolved this<br/> month</th>
                                        <th className="text-uppercase text-center">pending</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
                                        <td >1</td>
                                        <td>2020 - 2021</td>
                                        <td className="text-center">0</td>
                                        <td className="text-center">33</td>
                                        <td className="text-center">31</td>
                                        <td className="text-center">2</td>
                                    </tr>
                                    <tr>
                                        <td>2</td>
                                        <td>2021 - 2022</td>
                                        <td className="text-center">2</td>
                                        <td className="text-center">99</td>
                                        <td className="text-center">98</td>
                                        <td className="text-center">1</td>
                                    </tr>
                                </tbody>
                            </table>
                            </div>
                        </div>
                    </div>
                   
                </div>
            </section>
        </div>
    );
}
export default InvestorComplaintsMain;