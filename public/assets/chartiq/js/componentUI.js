...
class BaseComponent extends HTMLElement {
	...
	static activate(node, e, params, setter) {
		console.log("activateactivate",node, e, params, setter)
 ...
 * </cq-item>
 */
console.log(CIQ.SignalIQ,"MarkersSample2")
CIQ.UI.Markers.prototype.showMarkers = function (node, type, markerType) {
	...
	if (type) {
		...
		if (implementation.activeLabels.includes(type)) {
			...
			console.log(implementation,"MarkersSample3")
...
class Chart {
	...
	createChartAndUI({ container, config }) {
		...
		//console.log("onChartReady createChart2",config,location.search)
		const stx = CIQ.ChartEngine.create({
			container: chartContainer,
			config,
			deferLoad: true
		});
		console.log("CREATED STX",stx)

		chartContainer.stx = stx;


	// 	chartContainer.stx.chart.yAxis.initialMarginBottom = 10;
	// //	chartContainer.stx.chart.yAxis.decimalPlaces = (this.segmentId == '13' || this.segmentId == '14') ? 4 : 2;
	// 	chartContainer.stx.chart.yAxis.noDraw = false;
	// 	chartContainer.stx.chart.yAxis.drawCurrentPriceLabel = true
	// 	chartContainer.stx.chart.yAxis.width = 80;
	// 	chartContainer.stx.chart.yAxis.minimumPriceTick = 50;
	// 	chartContainer.stx.chart.xAxis.noDraw = false;
	...
	initLookup(uiContext) {
		...
		uiContext.changeSymbol = function (data, cb) {
			data.idid=uiContext.config.chartId
			const { stx, loader } = uiContext;
		
		//	console.log(uiContext,"datadata uiContext",data)
			...
			window.parent['jiffyAPI'].changeSymbol(data,stx,config.chartId)
	...
	loadChart(uiContext) {
		...
		//console.log("internal load called",uiContext)
		const {
			stx,
			topNode,
			config: {
		...
		if (restore) {
			...
			CIQ.ChartEngine.restoreLayout(
				...
				function () {
					...
					if((!(location.pathname.indexOf('advanced-multi-charts')>-1) )){
						if (localStorage) {

							let chartLayout = localStorage.getItem("myChartLayoutchart"+0);
						  
							//console.log(chartLayout,"(this.isOnCompany&&(this.isOnCompany=='true'))(this.isOnCompany&&(this.isOnCompany=='true'))",(this.isOnCompany&&(this.isOnCompany=='true')))
					...
					if((!(location.pathname.indexOf('advanced-multi-charts')>-1) ) && !(location.pathname.indexOf('advanced-charts')>-1)){
						stx.chart.hideDrawings =true
						stx.layout.crosshair=true;
						console.log("stx.chartstx.chart",stx)
						stx.layout.headsUp={
							dynamic:true,
							floating:true,
							static:true
		...
		/* 				if(stx.layout){
							stx.layout.backupcrosshair=stx.layout.crosshair
							stx.layout.backupheadsUp=stx.layout.headsUp
							stx.layout.crosshair=true;
							stx.layout.headsUp={
								dynamic:true,
								floating:true,
						...
						/* if(stx.channel){
							stx.channel.crosshair=true;
							
						} */
					
						for (var p in stx.panels) {
							if (stx.panels[p].name == "chart") continue;
							stx.panels[p].hidden = true;
						}
						...
						for (var p in stx.layout.studies) {
						  ...
						  stx.setPeriodicity({ period: 1,interval:15, timeUnit: 'minute' })
				...
				/* 		if(stx.layout){
							if(stx.layout.backupcrosshair){
								stx.layout.crosshair = stx.layout.backupcrosshair
							}

							if(stx.layout.backupheadsUp){
								stx.layout.headsUp = stx.layout.backupheadsUp
							}
							
							
						} */

						stx.chart.hideDrawings =false

						for (var p in stx.panels) {
							if (stx.panels[p].name == "chart") continue;
							stx.panels[p].hidden = false;
						}
						for (var p in stx.layout.studies) {
							if (stx.layout.studies[p].name == "chart") continue;
				...
				//	console.log(multiChartId,"internal load called chartId ",chartId)
				},
				chartId,
				uiContext.config
			);
...
class Multichart {
	...
	//	console.log("updateLayout called")
		container.updateLayout = function () {
			...
			//		console.log("updateLayout called wrapper",wrapper)
					const chartId = CIQ.getFromNS(
						wrapper.querySelector("cq-context") || {},
						"CIQ.UI.context.config.chartId"
					);
