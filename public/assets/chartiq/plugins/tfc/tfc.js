...
CIQ.TFC.prototype.updateValues = function () {
	console.log(subs, "this.activeTrade updateValues", this.activeTrade)
	if (subs && !subs.tfc_shares_id2) {
		if (document.getElementById('tfc-shares-id2')) {
			
			
			const input = document.getElementById("tfc-shares-id2");
			if(input){
				subs.tfc_shares_id2 = true;
				input.removeEventListener("input", updateValue);
				input.addEventListener("input", updateValue);
				console.log("ENTERED HERE",input)
			}
	...
	if(subs&& !subs.tfc_shares_id2){
		if(document.getElementById('tfc-shares-id')){
			

			const input = document.getElementById("tfc-shares-id");
			if(input){
				input.removeEventListener("input", updateValue);
				input.addEventListener("input", updateValue);
				subs.tfc_shares_id=true;
	...
	if (tradesLikeForex) {
		...
		console.log(this.stx.chart.symbolObject,"modifyingOrder.in",this.modifyingOrder)

		let useLots=false
		if(this.stx&&this.stx.chart&&this.stx.chart.symbolObject&&this.stx.chart.symbolObject.SegmentId){
			useLots = !(this.stx.chart.symbolObject.MarketLot == 1 && this.stx.chart.symbolObject.SegmentId!=5 && this.stx.chart.symbolObject.SegmentId!=7)
		}

		//if()
		if(useLots){
			for (e = 0; e < this.elements.useSharesLabel.length; e++) {
				if(this.elements.useSharesLabel[e])
				this.elements.useSharesLabel[e].style.display = "none";

				if(this.elements.useUnitsLabel[e]){
					this.elements.useUnitsLabel[e].style.display = "";
					if(this.stx&&this.stx.chart&&this.stx.chart.symbolObject&&this.stx.chart.symbolObject.MarketLot&&this.elements.useUnitsLabel[e] && (this.elements.useUnitsLabel[e].getAttribute('id')=='lot_size')){
						this.elements.useUnitsLabel[e].innerHTML=`<span>1 LOT=`+this.stx.chart.symbolObject.MarketLot+`</span>`	
					}
					console.log(this.elements.useUnitsLabel[e],"this.elements.useUnitsLabel[e]")
			...
			// 	//this.elements.useSharesLabel[e].style.display = "none";
			// 	//this.elements.useUnitsLabel[e].style.display = "";
			// 	if(this.stx&&this.stx.chart&&this.stx.chart.symbolObject&&this.stx.chart.symbolObject.b5Data&&this.elements.useUnitsLabel[e] && (this.elements.useUnitsLabel[e].getAttribute('id')=='circuit_range')){
			// 		this.elements.useUnitsLabel[e].innerHTML=`<span>Range=`+this.stx.chart.symbolObject.b5Data.lowPriceRange+`-`+this.stx.chart.symbolObject.b5Data.highPriceRange+`</span>`	
			// 		this.elements.useUnitsLabel[e].style.display = "";
			// 	console.log(this.elements.useUnitsLabel[e],"this.elements.useUnitsLabel[e]")
			// 	}
			...
			// console.log("document.getElementById('lot_size')",document.getElementById('lot_size'))
			// if(this.stx&&this.stx.chart&&this.stx.chart.symbolObject&&this.stx.chart.symbolObject.MarketLot&&document.getElementById('lot_size')){
			// 	document.getElementById('lot_size').innerHTML=`<span>1 LOT=`+this.stx.chart.symbolObject.MarketLot+`</span>`	
			// }
	
			// if(this.stx&&this.stx.chart&&this.stx.chart.symbolObject&&this.stx.chart.symbolObject.b5Data&&document.getElementById('circuit_range')){
			// 	document.getElementById('circuit_range').innerHTML=`<span>`+this.stx.chart.symbolObject.b5Data.lowPriceRange+`-`+this.stx.chart.symbolObject.b5Data.highPriceRange+`</span>`
			// }
...
CIQ.TFC.prototype.modifyOpenOrder = function (openOrderMarker) {
	...
	console.log("modifyOpenOrder",openOrder)
...
//USER UPDATED : SETTING MODIFIED VALUE OF PRODUCT TYPE / ORDER TYPE

	if(document.getElementById("orderType"))
	document.getElementById("orderType").checked = openOrder&&openOrder.orderType=='Delivery'//false
...
CIQ.TFC.prototype.createOpenOrderMarker = function (
	...
	CIQ.safeClickTouch(
		...
		(function (self, whichMarker) {
			return function () {
				self.crosshairsOn();
			//	console.log(whichMarker,"modifyOpenOrder")
...
CIQ.TFC.prototype.clearActive = function () {
	...
	// this.elements.orderTypeVal.value = "";
...
CIQ.TFC.prototype.cancelOpenOrder = function () {
	...
	this.account.confirmOrder(this, order, function (confirmation) {
		...
		var orderTypeNode = confirmDialog.querySelector(".orderType");
		...
		if (order.tif == "DAY")
			...
			orderTypeNode.innerHTML = "";
			if (order.orderType == "Intraday")
				orderTypeNode.appendChild(CIQ.translatableTextNode(self.stx, "Intraday"));
			else if (order.orderType == "Delivery")
				orderTypeNode.appendChild(
					CIQ.translatableTextNode(self.stx, "Delivery")
				);
...
CIQ.TFC.prototype.createOrderFromGUI = function (action) {
	...
	if (
		...
		let e=document.getElementById("orderType")
		if(e)
		order.orderType=e.checked?'Delivery':'Intraday';
...
CIQ.TFC.prototype.createReplaceFromGUI = function () {
	...
	console.log("orderreplaceOrder this.modifyingOrder createReplaceFromGUI",this.modifyingOrder)
	order.extra=this.modifyingOrder.extra
	...
	order.orderType.old = this.modifyingOrder.orderType;
	order.orderType["new"] = order.orderType.old;
...
CIQ.TFC.prototype.confirmReplace = function (order) {
	...
	console.log("orderreplaceOrder confirmReplace",order)
	...
	var orderTypeOld = confirmReplaceDialog.querySelector(".orderTypeOld");
	...
	var orderTypeNew = confirmReplaceDialog.querySelector(".orderTypeNew");
	...
	orderTypeOld.innerHTML = order.orderType.old;
	if (order.orderType.old != order.orderType["new"]) {
		orderTypeOld.classList.remove("no-change");
		foundAChange = true;
	} else {
		orderTypeOld.classList.add("no-change");
	}
	orderTypeNew.innerHTML = order.orderType["new"];
	...
	function closure(self, order, submitButton) {
		return function () {
			...
			confirmReplaceDialog.querySelector(".processOrder").style.display =
				...
				//Replace Order
				order.whichVal="1"
				console.log("orderreplaceOrder",order)
...
CIQ.TFC.prototype.confirmOrder = function (order) {
	...
	this.account.confirmOrder(this, order, function (confirmation) {
		...
		var orderTypeNode = confirmOrder.querySelector(".orderType");
...
CIQ.TFC.prototype.confirmOrPlaceOrder = function (type, action) {
	...
	if (type == "order") {
		...
		console.log("orderreplaceOrder confirmOrPlaceOrder"+type," : ",order)
		if (this.config.skipConfirms) {
			order.whichVal="7"
			this.placeOrder(order);
		} else {
...
CIQ.TFC.prototype.confirmOCO = function (order, type) {
	...
	var orderTypeNode = confirmOCO.querySelector(".orderType");
	...
	if (order[0].tif == "DAY")
		...
		orderTypeNode.innerHTML = "";
		if (order[0].orderType == "Intraday")
			orderTypeNode.appendChild(CIQ.translatableTextNode(this.stx, "Intraday"));
		else if (order[0].orderType == "Delivery")
			orderTypeNode.appendChild(
				CIQ.translatableTextNode(this.stx, "Delivery")
			);
...
CIQ.TFC.prototype.userAction = function (action) {
	...
	//HERE
	if (action == "replace") {
		console.log("userAction1",action)
		this.confirmOrPlaceOrder("replace");
	} else if (action == "OCO") {
		console.log("userAction2",action)
		...
		if (
			...
			console.log("userAction3",action)
			this.confirmOrPlaceOrder("bracket", action);
		} else {
			console.log("userAction4",action)
