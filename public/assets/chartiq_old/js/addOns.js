...
CIQ.RangeSlider =
	...
	function (params) {
		...
		stx.addEventListener("symbolChange", function (obj) {
			...
			if(location.pathname.indexOf('advanced-charts')>-1 && location.pathname.indexOf('advanced-charts-watch')==-1 && obj.symbolObject){
				console.log("symbolChange",obj)
				if(location.pathname!='/cq/advanced-charts/' + obj.symbolObject.Symbol + '/' + obj.symbolObject.SegmentId + '/' + obj.symbolObject.Token)
				history.pushState({}, null, '/cq/advanced-charts/' + obj.symbolObject.Symbol + '/' + obj.symbolObject.SegmentId + '/' + obj.symbolObject.Token);
			}
			else if(location.pathname.indexOf('advanced-charts-watch')>-1 && obj.symbolObject){
				console.log("symbolChange",obj)
				if(location.pathname!='/cq/advanced-charts-watch/' + obj.symbolObject.Symbol + '/' + obj.symbolObject.SegmentId + '/' + obj.symbolObject.Token)
				history.pushState({}, null, '/cq/advanced-charts-watch/' + obj.symbolObject.Symbol + '/' + obj.symbolObject.SegmentId + '/' + obj.symbolObject.Token);
			}else{
				let curr=stx.getSymbols()
				//console.log("symbol change 121212 stx",stx)
				//console.log("symbol change 121212",curr&&curr.length?curr[0]:' nope')
				//window.parent.jiffyAPI.changeSymbol(obj.symbolObject);
				/* for (let i = 0; i < 4; i++) {
					if (this.config.symbols['chart' + i])
					  symbols.push(this.config.symbols['chart' + i].SegmentId + '@' + this.config.symbols['chart' + i].Token)
				  } */
				  /* if (symbols.length) {
					this.location.replaceState('/advanced-multi-charts?symbols=' + symbols.join('|'));
				  } */
			}
		...
		stx.addEventListener("symbolImport", function (obj) {
			//console.log("symbolImport",obj)
			// if(obj.symbol&&obj.symbolObject&&obj.symbolObject.Token && location.pathname.indexOf('advanced-charts')>-1 && (location.pathname.indexOf(obj.symbolObject.Token)==-1)&& (location.pathname.indexOf(obj.symbolObject.symbol)==-1) ){
